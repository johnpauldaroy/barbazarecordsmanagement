-- Restore TUPAD if it was hidden by the old archive action.
...
update public.program_requirements pr
set
  archived_at = null,
  updated_at = now()
...
-- Keep disabled programs out of queue labels shown in dashboard widgets.
...
select
  ...
  string_agg(distinct sp.name, ', ' order by sp.name)
    filter (where sp.status = 'active' and sp.archived_at is null) as program_names,

-- Replaces transition_application_for_review with a quota-aware version.
-- Before approving or releasing an application, checks all programs on that
-- application against the active annual quota for the application's barangay.
-- If any program's quota is exhausted, the transition is hard-blocked.
-- Depends on: 20260521100000_barangay_program_quotas.sql
...
begin
  ...
  -- ── QUOTA CHECK ─────────────────────────────────────────────────────────────
  -- Only enforced when transitioning to 'approved' or 'released'.
  -- Checks every program on the application against its barangay quota.
  if v_target_status in ('approved', 'released') then
    v_current_year := extract(year from now())::integer;

    for v_program_id, v_program_name in
      select ap.program_id, sp.name
      from public.application_programs ap
      join public.social_programs sp on sp.id = ap.program_id
    ...
    loop
      ...
      into
        ...
        v_quota_active,
        v_quota_exists
      from public.check_barangay_quota(
        v_application.barangay_id,
        v_program_id,
        v_current_year
      ) q;

      if v_quota_exists and v_quota_active and v_quota_remaining <= 0 then
        raise exception
          'Quota reached: Barangay has used all % allocated slots for % in %. Cannot approve.',
          v_quota_max,
          v_program_name,
          v_current_year;
      end if;

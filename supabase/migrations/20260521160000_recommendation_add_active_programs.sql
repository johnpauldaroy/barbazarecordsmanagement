-- Adds active_program_names to get_recommendation_candidates RPC.
-- This field lists other programs the household is currently active in
-- (non-rejected, non-cancelled, current year) so the frontend can show
-- a warning note on the recommendation row.
...
as $$
  ...
  candidate_households as (
    select
      ...
      -- Collect names of other active programs this household is enrolled in this year
      (
        select array_agg(distinct sp2.name order by sp2.name)
        from public.applications a2
        join public.application_programs ap2 on ap2.application_id = a2.id
        join public.social_programs sp2 on sp2.id = ap2.program_id
        where a2.household_id = h.id
          and a2.archived_at is null
          and a2.current_status not in ('rejected', 'cancelled')
          and extract(year from coalesce(a2.decided_at, a2.submitted_at, a2.created_at))::integer = p_year
      ) as active_program_names
  ...
  select
    ...
    coalesce(s.active_program_names, array[]::text[]) as active_program_names,
    q.quota_id,
